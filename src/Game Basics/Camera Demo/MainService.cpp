...
{
    // Configure line renderer to use out camera's transformation.
    SetViewTransform( GetCamera().GetMatrix() );

    // Visualize visible area.
    // SetDrawColor(RalColors::TrafficWhite);
    // DrawRectangle(0, 0, GetCamera().GetViewWidth() * 0.99f, GetCamera().GetViewHeight() * 0.99f);
...
{
    shared_ptr<VertexBuffer2f> triangleShape = ShapeGenerator().GenTriangle(0.5f);
